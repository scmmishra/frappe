<template>
	<div class="modules-page-container">
		...
		<div v-for="category in module_categories" :key="category">
			<desk-section
...
export default {
	...
	data() {
		let modules_list = frappe.boot.allowed_modules.filter(
	...
	methods: {
		get_module_count(module_name) {
			var module_doctypes =
				frappe.boot.notification_info.module_doctypes[module_name]
			var sum = 0
			...
			if (module_doctypes && frappe.boot.notification_info.open_count_doctype) {
				...
				for (var j = 0, k = module_doctypes.length; j < k; j++) {
					...
					let count =
						frappe.boot.notification_info.open_count_doctype[doctype] || 0
			...
			if (
				frappe.boot.notification_info.open_count_doctype &&
				frappe.boot.notification_info.open_count_doctype[module_name] != null
			) {
				// notification count explicitly for doctype
				let count =
					frappe.boot.notification_info.open_count_doctype[module_name] || 0
				count = typeof count == 'string' ? parseInt(count) : count
				sum += count
			}
			...
			if (
				...
				let count =
					frappe.boot.notification_info.open_count_module[module_name] || 0
				count = typeof count == 'string' ? parseInt(count) : count
				sum += count
			}
